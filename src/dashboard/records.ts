...
export function projectCallRecord(text: string, expectedId: string): LocalCall {
  ...
  for (const line of lines) {
    if (Buffer.byteLength(line) > 64 * 1024) throw new AppError("dashboard_record_event_too_large");
    let value: unknown;
    ...
    if (Date.parse(timestamp) < Date.parse(call.startedAt)) throw new AppError("dashboard_invalid_record");
    call.lastEventAt = timestamp;
    if (type === "transcript") {
      // Only count events. Neither text nor acoustic timing leaves the private record.
...
export class DashboardRecords {
  ...
  constructor(private readonly directory: string, private readonly days: number) {}
  ...
  async read(now = new Date()): Promise<RecordSummary> {
    ...
    try {
      ...
      let restrictedPermissions = (stat.mode & 0o077) === 0;
      ...
      for (const { name, id } of selected) {
        const file = await open(`${path}/${name}`, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
        try {
          const info = await file.stat();
          if (!info.isFile() || info.nlink !== 1 || info.uid !== process.getuid?.() || info.size > 8 * 1024 * 1024) {
          ...
          if (previous?.signature === signature) calls.push(previous.call);
            ...
            const call = projectCallRecord(buffer.toString("utf8"), id);
            this.cache.set(name, { signature, call });
            calls.push(call);
          }
        } finally { await file.close(); }
      }
      return { calls, limited: names.length > limit, limit, restrictedPermissions };
    } catch (error) {
