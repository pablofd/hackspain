...
| Public caller handoff / silence (19 Sep 03:51 UTC) | A date practice stopped twice before any clinic tool: first with zero caller input, then after our identity question. Prosper's public transcript contains the complete outgoing question, but no subsequent patient response. Our local transport/model recorded no error. Thus reception/STT worked on the final attempt; downstream turn completion or caller generation remains unverified. | Trial a four-second bounded output-silence tail instead of 1.2 seconds, retaining immediate preemption by speech and unchanged queue/call limits. Wire tests measure exactly 200 paced silent frames. This is a compatibility experiment, not proof that Prosper is at fault or that a longer tail fixes the case; repeat the same public practice before scoring. |
